...
(() => {
  ...
  const fileToDataUrl=file=>new Promise(resolve=>{if(!file||file.size>2*1024*1024)return resolve(null);const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>resolve(null);reader.readAsDataURL(file);});
  ...
  async function upvote(issueId){if(!API||!issueId||String(issueId).startsWith('demo-'))return null;try{const r=await fetch(`${API}/api/issues/${encodeURIComponent(issueId)}/upvote`,{method:'POST',headers:{'X-Device-ID':getDeviceId(),Accept:'application/json'}});const body=await r.json().catch(()=>({}));return r.ok||r.status===409?body:null;}catch{return null;}}
  async function verify(issueId,verdict){if(!API||!issueId||String(issueId).startsWith('demo-'))return null;try{const r=await fetch(`${API}/api/issues/${encodeURIComponent(issueId)}/verify`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({verdict})});return r.ok?await r.json():null;}catch{return null;}}
