(() => {
  ...
  async function json(url, options={}) {
    ...
    if(!r.ok) throw Error(body?.error||`Request failed (${r.status})`);
    return body;
  }
  ...
  function ordered(items) {
    // Keep the seeded Delhi cases visible even when the API returns many reports.
    return [...items].sort((a,b) => Number(Boolean(b.demo)) - Number(Boolean(a.demo)) || Number(b.priority||0) - Number(a.priority||0));
  }

  function show(items) {
    const box=$('#issue-list');
    const visible=ordered(items);
  ...
  async function loadIssues(){
    ...
    try {
      ...
      console.warn('KOINOS API unavailable:',e);
    }
  ...
  function renderLive(status){
    ...
    markers=ordered(all).filter(i=>Number.isFinite(Number(i.latitude))&&Number.isFinite(Number(i.longitude))).map(marker);
    markers.forEach(m=>m.addTo(map));
    show(all);
    $('#updated').textContent=new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
  ...
  async function directStreetImagery(){
    const url=`${KARTA}?lat=${center[0]}&lng=${center[1]}&radius=12000&zoomLevel=12&join=sequence&orderBy=id&orderDirection=desc`;
    const b=await json(url); const rows=b?.result?.data||b?.data||[];
    return rows.map(x=>({image:pickImage(x),capturedAt:pickDate(x),lat:Number(x.lat??x.latitude),lng:Number(x.lng??x.longitude),sourceUrl:x.id?`https://kartaview.org/details/${x.id}`:'https://kartaview.org/map/@28.6139,77.209,12z'})).filter(x=>x.image&&Number.isFinite(x.lat)&&Number.isFinite(x.lng)).filter(x=>x.lat>28.35&&x.lat<28.9&&x.lng>76.8&&x.lng<77.6).slice(0,6);
  }
  ...
  async function street(){
    ...
    try{
      ...
      if(!rows.length)rows=await directStreetImagery();
      if(!rows.length){host.innerHTML='<div class="notice">No public street-level capture is available for this Delhi area right now.</div>';return;}
      host.innerHTML=rows.slice(0,6).map(x=>`<article class="issue"><img class="issue-img street-photo" src="${esc(x.image)}" alt="Public street-level capture" loading="lazy"><div class="mono" style="margin-top:10px">${esc(x.capturedAt||'Capture date unavailable')}</div><div class="muted">${Number(x.lat).toFixed(4)}, ${Number(x.lng).toFixed(4)}</div><a class="btn" style="display:inline-block;margin-top:10px" target="_blank" rel="noopener noreferrer" href="${esc(x.sourceUrl||'https://kartaview.org/')}">Open KartaView ↗</a></article>`).join('');
    }catch(e){console.warn('Street imagery unavailable:',e);host.innerHTML='<div class="notice">Street imagery is temporarily unavailable. The civic map and public case references are still available.</div>';}
  }

  function apply(cat){
    const filtered=cat==='all'?all:all.filter(i=>(i.category||'').toLowerCase().includes(cat));
    show(filtered);
    markers.forEach(m=>{const ok=cat==='all'||m.__issue?.category===cat;m.setStyle({opacity:ok?1:.15,fillOpacity:ok?1:.15});});
  }
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    await loadIssues();
    street();
    document.querySelectorAll('[data-cat]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-cat]').forEach(x=>x.classList.remove('active'));b.classList.add('active');apply(b.dataset.cat);});
    $('#locate').onclick=()=>navigator.geolocation?.getCurrentPosition(p=>map.setView([p.coords.latitude,p.coords.longitude],15));
