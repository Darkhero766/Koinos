...
(() => {
  ...
  if (!document.querySelector('link[data-koinos-mobile]')) {
    const css = document.createElement('link'); css.rel = 'stylesheet'; css.href = '/mobile.css'; css.dataset.koinosMobile = 'true'; document.head.appendChild(css);
  }
...
$('.join-issue-button')?.addEventListener('click', async () => { if (window.KOINOS_LIVE?.submitReport) await window.KOINOS_LIVE.submitReport(); $$('.report-step').forEach(x => x.classList.remove('active')); $('#modal-success')?.classList.add('show'); toast('Your report is now part of the community record.'); });
$('.separate-button')?.addEventListener('click', async () => { if (window.KOINOS_LIVE?.submitReport) await window.KOINOS_LIVE.submitReport(); toast('Report saved.'); closeReport(); });
$('.close-success')?.addEventListener('click', closeReport);
$('.support-button')?.addEventListener('click', async e => { if (e.currentTarget.dataset.supported) return; const issueId=e.currentTarget.dataset.issueId; const result=issueId&&window.KOINOS_LIVE?.upvote?await window.KOINOS_LIVE.upvote(issueId):null; const count=result?.upvotes ?? (Number($('#affected-count')?.textContent||47)+1); if($('#affected-count')) $('#affected-count').textContent=count; if($('.support-note')) $('.support-note').textContent=`${count} people have confirmed this problem.`; e.currentTarget.dataset.supported='true'; e.currentTarget.innerHTML="You're affected too <span>✓</span>"; toast('Your voice was added to this community issue.'); });
...
const comparison=$('.comparison-slider'); const updateComparison=()=>{if(!comparison)return; $('.damaged')&&($('.damaged').style.width=`${comparison.value}%`); $('.slider-handle')&&($('.slider-handle').style.left=`${comparison.value}%`);}; comparison?.addEventListener('input',updateComparison); updateComparison();
...
window.addEventListener('koinos:issue',e=>{const issue=e.detail||{},card=$('#selected-issue');if(!card)return;card.querySelector('h3')&&(card.querySelector('h3').textContent=issue.title||'Civic issue');card.querySelector('> p')&&(card.querySelector('> p').textContent=issue.locationLabel||issue.address||'Nearby');const metrics=$$('.issue-metrics strong',card);if(metrics[0])metrics[0].textContent=issue.upvotes??0;if(metrics[1])metrics[1].textContent=issue.priority??issue.severity?.toUpperCase()??'—';if(metrics[2])metrics[2].textContent=issue.reportCount??1;$('.support-button')?.setAttribute('data-issue-id',issue.id||'');});
